-- VidiPay 100x scaling SQL.
-- Safe additive migration: no deletes, no drops.
-- Run after the 3M SQL if it has not already been applied.

select now() as vidipay_100x_sql_started_at;
...
begin
  ...
  execute 'create index if not exists idx_payment_scanner_heartbeats_live_100x
    on public.payment_scanner_heartbeats (network, token, worker_mode, last_seen_at desc)';

  execute 'create index if not exists idx_payment_scanner_heartbeats_shard_100x
...
begin
  if to_regclass('public.payment_orders') is null then
    raise notice 'Skipped 100x payment order SQL: public.payment_orders table does not exist.';
  else
  ...
  if to_regclass('public.payment_transactions') is not null then
    execute 'create index if not exists idx_payment_transactions_created_100x
      on public.payment_transactions (created_at desc)';
    execute 'create index if not exists idx_payment_transactions_wallet_created_100x
  ...
  if to_regclass('public.view_logs') is not null then
    execute 'create index if not exists idx_view_logs_created_100x
      on public.view_logs (created_at desc)';
    execute 'create index if not exists idx_view_logs_user_created_100x
...
declare
  ...
  v_shard_count integer := least(greatest(coalesce(p_shard_count, 1), 1), 256);
...
select
  'vidipay_100x_sql_finished' as check_name,
  ...
  ) as heartbeat_scan_jitter_ready,
  now() as finished_at;
