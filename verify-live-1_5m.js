...
function main() {
  ...
  if (mode === "api" && rateLimitBackend !== "redis") {
    errors.push("RATE_LIMIT_BACKEND must be redis for 100x API mode");
  }
  ...
  if (mode === "scanner" && !value("REDIS_URL")) {
    warnings.push("REDIS_URL is empty; scanner can still run, but API service should use Redis for 100x traffic");
  }
  ...
  if (mode === "scanner") {
    ...
    checkPositiveNumber(errors, "PAYMENT_SCAN_INTERVAL_MS", { min: 2000, max: 300000 });
    checkPositiveNumber(errors, "PAYMENT_SCAN_BATCH_SIZE", { min: 1, max: 1000 });
    checkPositiveNumber(errors, "PAYMENT_SCAN_CONCURRENCY", { min: 1, max: 64 });
    checkPositiveNumber(errors, "PAYMENT_SCAN_JITTER_MS", { min: 0, max: 60000 });
    checkPositiveNumber(errors, "PAYMENT_SCANNER_SHARD_COUNT", { min: 1, max: 256 });
    checkPositiveNumber(errors, "PAYMENT_SCANNER_SHARD_INDEX", { min: 0, max: 255 });
  ...
  console.log(`VidiPay 100x env check mode: ${mode}`);
